...
static Ring1_Result Stack_initialize(Stack* self) {
  self->elements = NULL;
  if (Ring1_Memory_allocateArray(&self->elements, 8, sizeof(Ring2_Value))) {
    return Ring1_Result_Failure;
  }
  for (size_t i = 0; i < 8; ++i) {
    Ring2_Value_setVoid(self->elements + i, Ring2_Void_Void);
  }
  self->size = 0;
  self->capacity = 8;
...
static void Stack_grow(Stack* self, size_t additionalCapacity) {
  ...
  if (Ring1_Memory_recomputeSize_sz(0, SIZE_MAX / sizeof(Ring2_Value), self->capacity,
                                    additionalCapacity, &newCapacity, true)) {
    Ring2_jump();
  }
  Ring2_Value* newElements = NULL;

  ...
  )
{
  ...
  unevaluatedNode = Mkx_Intrinsic_cast(Mkx_Scribe_Cas_Node*, Mkx_Scribe_Cas_BinaryPlusNode_create(context, leftOperand, rightOperand));
  ...
  )
{
  ...
  RING1_TEST_ASSERT_EQ(ctx, 0, Mkx_Scribe_Cas_Test_safeExecute(context, &matrixAddition_1x1));
  RING1_TEST_ASSERT_EQ(ctx, 0, Mkx_Scribe_Cas_Test_safeExecute(context, &matrixAddition_2x2));
  ...
  )
{
  ...
  if (Ring1_Unlikely(Ring1_Test_Context_addTest(ctx, "Mkx.Scribe.Cas.Test.MatixAdditionTests", &test))) {
    return Ring1_Result_Failure;
  }

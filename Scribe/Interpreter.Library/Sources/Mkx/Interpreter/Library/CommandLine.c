...
Ring2_CommandLine_Argument_create
  (
    Ring1_CommandLine_Argument* pimpl
  );
...
{
  if (self->pimpl) {
    Ring1_CommandLine_Argument_unref(self->pimpl);
    self->pimpl = NULL;
  }
}
...
{
  if (!self) {
    fprintf(stderr, "%s:%d: assertion `%s` failed\n", __FILE__, __LINE__, "NULL != self");
    Ring1_Status_set(Ring1_Status_InvalidArgument);
    Ring2_jump();
  }
  char const* p;
  if (Ring1_CommandLine_Argument_getName(&p, self->pimpl)) {
    Ring2_jump();
  }
  ...
  )
{
  ...
  char const* p;
  if (Ring1_CommandLine_Argument_getValue(&p, self->pimpl)) {
    Ring2_jump();
  }
  return Ring2_String_fromC(false, p);
}
...
{
  if (self->pimpl) {
    Ring1_CommandLine_ArgumentList_unref(self->pimpl);
    self->pimpl = NULL;
  }
}
  ...
  )
{
  ...
  Ring1_CommandLine_ArgumentList* pimpl = Ring1_CommandLine_ArgumentList_parse(argc, argv);
  if (!pimpl) {
    fprintf(stderr, "%s:%d: %s failed\n", __FILE__, __LINE__, "Ring1_CommandLine_ArgumentList_parse");
    Ring2_jump();
  }
  self->pimpl = pimpl;
  ...
  )
{
  ...
  Ring1_CommandLine_Argument* pimpl;
  if (Ring1_CommandLine_ArgumentList_getAt(&pimpl, self->pimpl, index)) {
    fprintf(stderr, "%s:%d: %s failed\n", __FILE__, __LINE__, "Ring1_CommandLine_ArgumentList_getAt");
    Ring2_jump();
  }
  Ring2_JumpTarget jt;
  ...
  if (!setjmp(jt.environment)) {
    ...
    Ring2_popJumpTarget();
    Ring1_CommandLine_Argument_unref(pimpl);
    pimpl = NULL;
    Ring2_jump();
  }
}
  ...
  )
{
  ...
  int64_t size;
  if (Ring1_Unlikely(Ring1_CommandLine_ArgumentList_getSize(&size, self->pimpl))) {
    fprintf(stderr, "%s:%d: %s failed\n", __FILE__, __LINE__, "Ring1_CommandLine_ArgumentList_getSize");
    Ring2_jump();
  }
  return size;
}
  ...
  )
{
  ...
  if (Ring1_CommandLine_ArgumentList_removeAt(self->pimpl, index)) {
    fprintf(stderr, "%s:%d: %s failed\n", __FILE__, __LINE__, "Ring1_CommandLine_ArgumentList_removeAt");
    Ring2_jump();
  }
}

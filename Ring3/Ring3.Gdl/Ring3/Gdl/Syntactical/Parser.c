  ...
  )
{
  ...
  self->scanner = Machine_Gdl_Scanner_create(Ring2_String_fromC(false, "<empty input>"), Ring2_ByteBuffer_create());
...
checkKind
  (
    Machine_Gdl_Parser* self,
    Machine_Gdl_TokenKind expected
  )
{
...
{
  if (currentKind(self) != Machine_Gdl_TokenKind_Name) {
    Ring1_Status_set(Ring1_Status_InvalidSyntactics);
    Ring2_jump();
  }
  Ring2_String* nameString = Machine_Gdl_Scanner_getTokenText(self->scanner);
  ...
  )
{
  ...
  case Machine_Gdl_TokenKind_Boolean: {
    Ring2_String* literalString = Machine_Gdl_Scanner_getTokenText(self->scanner);
